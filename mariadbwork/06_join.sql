...
-- 4) cross join 의도한 경우보다 on에 쓰이는 join조건을 잘못 썼을 가능성 有

SELECT 
		 *
		FROM tbl_menu a
		LEFT JOIN tbl_category b ON (1=1); -- -> (1=1) 모든 경우의 수 곱셈 처리
...
-- 5) self join
-- self join은 하나의 테이블에 상위 개념과 하위 개념이 모두 드어있을 때 자기 자신 테이블을 join하게 되는 경우 사용
-- ex) 신입사원 vs 사수(선배사원)
-- 	하위카테고리 vs 상위 카테도리
-- 	댓글 vs 대댓글 
